...
_flutter.buildConfig = {"engineRevision":"83675ed27633283e7fc296c8bca22e841224c096","builds":[{"compileTarget":"dart2wasm","renderer":"skwasm","mainWasmPath":"main.dart.wasm","jsSupportRuntimePath":"main.dart.mjs"},{"compileTarget":"dart2js","renderer":"canvaskit","mainJsPath":"main.dart.js"}]};


// Flutter's generated service worker can keep an older 3D runtime alive on a
// localhost origin after a rebuild. Migrate that cache once, then boot without
// registering another service worker so local QA always runs the current files.
(async () => {
  const migrationKey = 'waxball-web-cache-version';
  const migrationVersion = 'graphics-runtime-v1';
  const localQaHosts = new Set(['localhost', '127.0.0.1', '[::1]']);
  const isLocalQaHost = localQaHosts.has(window.location.hostname);
  ...
  if (
    ...
    try {
      ...
      console.log('Waxball web cache migrated: graphics-runtime-v1');
    } catch (error) {
      console.warn('Waxball web cache migration skipped:', error);
    }
  }

  // A controlling legacy worker or the HTTP memory cache may still retain the
  // old entrypoint for the current page load. A versioned URL is not part of
  // Flutter's old resource table, so it is guaranteed to reach this server.
  for (const build of _flutter.buildConfig.builds) {
    if (build.mainJsPath) {
      // Do not use Flutter's conventional `?v=` parameter: legacy workers
      // deliberately strip it before cache lookup. This app-specific key is
      // left intact and therefore bypasses their old resource table.
      build.mainJsPath =
        `${build.mainJsPath}?waxball_runtime=${migrationVersion}`;
    }
  }
